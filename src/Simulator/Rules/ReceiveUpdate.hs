...
receiveUpdate acqua  =
  ...
  in case m of
      Just (ConstMsgUpdate (MsgUpdate pId pointerS idx val) 0) -> trace ((show (PU.puId pu)) ++ ": receive update")  $ receiveUpdate (acqua { processingUnits = pus', interconnection = iret, finishFlag = f' })
        where
          Just m' = m
