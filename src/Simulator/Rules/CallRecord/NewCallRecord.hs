module Simulator.Rules.CallRecord.NewCallRecord where
      ...
      case (PU.commands pu,PU.canExecuteCmds pu) of
        (((NewCallRecord x n):cs),True) -> trace ((show (PU.puId pu)) ++ ": NewCallRecord " ++ (show x) ++ " " ++ (show n)) pu'
          where
            ...
            cr = CallRecordV $ emptyCallRecord { params = (Sequence.replicate n (NumberV 0)) }
            crseg' = Map.insert crsegPos cr crseg
