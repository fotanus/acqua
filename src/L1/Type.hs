...
inferType n (Op e1 _ e3) nameTypes = if e1 == (Ident n) || e3 == (Ident n)
                                        then IntT
...
inferType n (If e1 e2 e3) nameTypes = if e1 == (Ident n)
                                      then IntT
                                      else if inferType n e2 nameTypes == UnknownT
                                           then inferType n e3 nameTypes
                                           else inferType n e2 nameTypes
inferType n (Let _ e1 e2) nameTypes = if inferType n e1 nameTypes == UnknownT
                                      then inferType n e2 nameTypes
                                      else inferType n e1 nameTypes
inferType n (Letrec _ e1 e2) nameTypes = if inferType n e1 nameTypes == UnknownT
                                         then inferType n e2 nameTypes
                                         else inferType n e1 nameTypes
...
typeCheck (App e1 e2) nameTypes = let
...
typeCheck (Op e1 _ e2) nameTypes = if isTypeOrUnknown (typeCheck e1 nameTypes) IntT
...
typeCheck (If e1 e2 e3) nameTypes = if isTypeOrUnknown (typeCheck e1 nameTypes) IntT
