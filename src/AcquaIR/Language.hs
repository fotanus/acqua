...
data Command
  ...
  | GetClosureParam Name Name Name
  | Wait
  deriving (Eq,Ord,Show,Read)
...
printCommand (NewClosure n1 i) = ident ++ n1 ++  " = " ++ "NewClosure " ++ (show i) ++ "\n"
...
printCommand (GetClosureParam n1 n2 n3) = ident ++ n3 ++ " = GetClosureParam " ++ n1 ++ " " ++ n2 ++ "\n"
