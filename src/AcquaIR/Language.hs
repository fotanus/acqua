...
data BasicBlock = BB {size :: Int, commands :: [Command]}
...
printCommand (Goto l) = ident ++ "goto " ++ l ++ "\n"
printCommand (Return name) = ident ++ "return " ++ name ++ "\n"
printCommand (If name l) = ident ++ "if " ++ name ++ " goto " ++ l ++ "\n"
